...
class CVClassifier:
    def __init__(self, blob_service_client):
        ...
        self.trainer = CustomVisionTrainingClient(ENDPOINT, self.training_credentials)
    ...
    def upload_images(self, labels: list):
        ...
        for label in labels:
            ...
            try:
                container = blob_service_client.get_container_client(str(label))
            except Exception as e:
                print("could not find container with label " + label + " error: ", e)

            for blob in container.list_blobs():
                blob_name = blob.name
                blob_url = f"{base_img_url}/{label}/{blob_name}"
                url_list.append(ImageUrlCreateEntry(url=blob_url, tag_ids=[tag.id]))
